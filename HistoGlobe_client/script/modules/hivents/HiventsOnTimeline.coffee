...
class HG.HiventsOnTimeline
  ...
  hgInit: (@_hgInstance) ->
      ...
      @_hiventController.getHivents @, (handle) =>
        show = (self, oldState) =>
          if oldState is 0 # invisible
            ...
            marker = new HG.HiventMarkerTimeline @_hgInstance, @_timeline, self, @_timeline.getSlider().elem(), @_timeline.dateToPosition(hiventMarkerDate), rowPosition, id
    ...
    #       marker = new HG.HiventMarkerTimeline @_timeline, handle, @_timeline.getSlider().elem(), @_timeline.dateToPosition(hiventMarkerDate)
    #       callback marker for callback in @_onMarkerAddedCallbacks

    #       '''show = (self, oldState) =>
    #         if oldState is 0 # invisible
    #           hiventMarkerDate = self.getHivent().startDate
    #           marker = new HG.HiventMarkerTimeline @_timeline, self, @_timeline.getSlider().elem(), @_timeline.dateToPosition(hiventMarkerDate)
    #           @_hiventMarkers.push marker
    #           @_markersLoaded = @_hiventController._hiventsLoaded
    #           callback marker for callback in @_onMarkerAddedCallbacks
